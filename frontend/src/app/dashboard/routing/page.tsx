...
export default function RoutePlanning() {
  ...
  const [areaFamCount, setAreaFamCount] = useState(0);
  ...
  useEffect(() => {
    ...
    if (savedGuests) {
      ...
      const uniqueAreas = Array.from(new Set(gList.map(g => `${g.city} - ${g.village || "General"}`)));
  ...
  const loadMarkers = (L: any) => {
    ...
    const fams = areaGuests.reduce((s, g) => s + (g.familyMembers || 1), 0);
    setAreaFamCount(fams);
    ...
    areaGuests.forEach(g => {
      ...
      let pinColor = "#DC2626"; // red
      if (g.status === "Assigned") pinColor = "#D97706"; // orange
      if (g.status === "Distributed") pinColor = "#059669"; // green
      ...
      // Create window-accessible function to mark distributed
      (window as any).markDistributedFromMap = (id: number) => {
        ...
        const updated = guests.map(item => {
          if (item.id === id) {
            return {
              ...
              remarks: "Delivered via map pin action."
            };
          }
          return item;
        });
      ...
      const popupHtml = `
        <div class="p-2 text-xs font-montserrat space-y-1.5" style="min-width: 180px;">
          ...
          ${g.status !== 'Distributed' ? `
            <button onclick="window.markDistributedFromMap(${g.id})" class="w-full btn-gold py-1.5 rounded mt-2 text-[10px] font-semibold text-center uppercase tracking-wider">
              Delivered Just Now
            </button>
      ...
      <div className="bg-cream-light border border-gold-foil/30 p-5 rounded-lg shadow flex flex-col justify-between space-y-4 lg:col-span-1">
        <div>
          ...
          <div className="space-y-4 text-xs">
            ...
            <div className="bg-maroon/5 border border-gold/20 p-3 rounded space-y-2 mt-4">
              <div className="font-cinzel text-[10px] font-bold text-maroon-dark">Area Statistics</div>
              <div className="flex justify-between">
                <span className="text-gray-600">Guests in Area:</span>
                <span className="font-bold text-maroon">{areaGuestsCount}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Family Members:</span>
                <span className="font-bold text-maroon">{areaFamCount}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Estimated Distance:</span>
                <span className="font-bold text-gold-dark">{estDistance}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-[9px] text-gray-500 text-center italic">
            * Calculates travel path based on nearest-neighbor coordinates.
          </p>
        </div>
      </div>
        ...
        <div className="bg-cream border-t border-gold/20 p-2.5 flex flex-wrap items-center justify-between text-[10px] text-maroon-dark">
          <div className="flex space-x-4">
            <span className="flex items-center">
              <span className="w-3 h-3 rounded-full bg-red-600 border border-white mr-1 shadow-sm"></span> 
              Pending Cards
            </span>
            <span className="flex items-center">
              <span className="w-3 h-3 rounded-full bg-amber-500 border border-white mr-1 shadow-sm"></span> 
              Assigned Cards
            </span>
            <span className="flex items-center">
              <span className="w-3 h-3 rounded-full bg-emerald-600 border border-white mr-1 shadow-sm"></span> 
              Distributed Cards
            </span>
          </div>
          <div>
            <span className="italic">Tip: Click marker to see details or mark delivered.</span>
          </div>
        </div>
      </div>
